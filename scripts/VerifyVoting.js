...
async function main() {
    ...
    let anonymous = await voterContract[voterindex].getAopenlist(0);
    let count = 0;
    let valid = 0;
    let ins1 = await voterContract[voterindex].getAresultCommit(anonymous);
    for(let i =0; i < vn; i++){
      if(i == voterindex) continue;
      let ins2 = await voterContract[voterindex].getAresultYi(anonymous, addr[i].address);
      if(count*2 > vn) {
        valid = 1;
        break;
      }
