...
describe("DTVoting", function () {
  ...
  describe("Function Tests", function () {
    ...
    it("Should initialize a voting contract correctly", async function () {
      ...
      //one cannot call the initialize method if he is not a owner of the contract
      await expect(
        voterContract[0].initialize(addr[0].address,idList[0],serialize(pklist[0]),serialize(pklists[0]))
      ...
      //cannot change a voter's information which has been initialized
      await expect(
        DTVoting.initialize(addr[1].address,idList[3],serialize(pklist[3]),serialize(pklists[3]))
      ).to.be.revertedWith("Transaction reverted without a reason string");
