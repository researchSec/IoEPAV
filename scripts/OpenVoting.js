...
async function main() {
    ...
    let commit_c = "4f486e501d03c175b20bbd80de61c02569309fe0ed2142d6affc4c399a52e05b";
    let commit_o = "327c8fb94dad6e2d41a005c2b87ef8dfff66f80d61c25b755b313daa5749e577";
    ...
    for(let i =0; i < vn; i++){
      if(i == voterindex) continue;
      let ins = await voterContract[voterindex].getDsigs(addr[voterindex].address,addr[i].address);
      let voter = ins[0];
      let dsi = ins[1];
      ...
      if(!verify_blind_signature(unserialize(commit_c),cvoter[i],pklist[i],yi)) {
        //console.log(i);
        await voterContract[voterindex].openVotesStart(addr[i].address,m,commit_c,commit_o,serialize(yi),serialize(cvoter[i]));
      }
    }

    await voterContract[voterindex].openVotesTermin(true);
