...
async function main() {
    ...
    let start = Date.now();
    //everyone can verify the valid of the initialization
    for(let i=0; i < vn; i++){
      let ins = await voterContract[0].getIdBasic(addr[i].address);
      if( addr[i].address != ins[0]||serialize(pklist[i]) != ins[1] || idList[i] != ins[2] || serialize(pklists[i]) != ins[3]){
    ...
    console.log(`time for verifying a voting contract = ${time} MS`);
